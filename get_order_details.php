...
if (!$order || (!$is_admin && $order['user_id'] != $user_id)) {
    echo "Bạn không có quyền xem chi tiết đơn hàng này!";
    exit();
}
...
$total = 0;
$output = '<table><thead><tr><th>Sản phẩm</th><th>Số lượng</th><th>Giá</th></tr></thead><tbody>';
...
$output .= '</tbody></table>';
$output .= "<p><strong>Tổng cộng: " . number_format($total, 0, ',', '.') . " VND</strong></p>";
